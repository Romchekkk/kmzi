...
/* Key Length */ //число 32-битных слов, составл€ющих шифроключ
#define Nk 4
/* Block Size */ // число столбцов (32-битных слов), составл€ющих State
#define Nb 4
/* Number of Rounds */ //число раундов, которое €вл€етс€ функцией Nk и Nb
#define Nr 10
...
/* State array */ //промежуточный результат шифровани€, который может быть представлен как пр€моугольный массив байтов, имеющий 4 строки и Nb колонок
static vector<vector<unsigned char>> stateMatrix(4, vector<unsigned char>(4));

/* All the Round Keys */ //Round Keys получаютс€ из Cipher Key использованием процедуры Key Expansion. ќни примен€ютс€ к State при шифровании и расшифровании 
static vector<unsigned char> roundKeys(Nb* Nk* (Nr + 1));
...
/* The S-box table */ //256 элементов //нелинейна€ таблица замен, использующа€с€ в нескольких трансформаци€х замены байтов и в процедуре Key Expansion дл€ взаимнооднозначной замены значени€ байта. 
static const vector<unsigned char> sbox = {
...
/* The round constant table (needed in KeyExpansion) */ //массив, который состоит из битов 32-разр€дного слова и €вл€етс€ посто€нным дл€ данного раунда.
static const vector<unsigned char> rcon = {
...
static void Print(vector<vector<unsigned char>> myarray) {
...
// 	функци€, используема€ в процедуре Key Expansion, котора€ берЄт на входе четырЄхбайтовое слово и, примен€€ S-box к каждому из четырЄх байтов, выдаЄт выходное слово 
static void SubWord(vector<unsigned char>& word) {
	word[0] = sbox[word[0]];
	word[1] = sbox[word[1]];
	word[2] = sbox[word[2]];
	word[3] = sbox[word[3]];
...
// функци€, использующа€с€ в процедуре Key Expansion, котора€ берЄт 4-байтовое слово и производит над ним циклическую перестановку 
static void RotWord(vector<unsigned char>& word) {
...
static void KeyExpansion128(const vector<unsigned char> key) {

	vector<unsigned char> temp(4); //[4]
	unsigned char i, j;
...
// трансформаци€ при шифровании и обратном шифровании, при которой Round Key XORТитс€ c State. ƒлина RoundKey равна размеру State 
static void AddRoundKey() {

	vector<vector<unsigned char>> tempRoundKeys(4, vector<unsigned char>(4));
	vector<unsigned char> tempArray(4);
	unsigned char i, j;
...
static void MixColumns() {
	...
	unsigned char i, j;
	vector<unsigned char> tempCell(4);
	...
	for (i = 0; i < Nb; i++)
	{
		tempCell[0] = (xtime(stateMatrix[0][i])
			...
			^ (stateMatrix[2][i])
			^ (stateMatrix[3][i]));			// ({02} Х s0c ) ({03} Х s1c ) (s2c) (s3c)

		tempCell[1] = (stateMatrix[0][i]
			^ (xtime(stateMatrix[1][i]))
			^ (xtime(stateMatrix[2][i]) ^ stateMatrix[2][i])
			^ (stateMatrix[3][i]));			// (s0c) ({02} Х s1c ) ({03} Х s2c) (s3c)

		tempCell[2] = (stateMatrix[0][i]
			^ stateMatrix[1][i]
			^ (xtime(stateMatrix[2][i]))
			^ (xtime(stateMatrix[3][i]) ^ stateMatrix[3][i]));	// (s0c) (s1c) ({02} Х s2c) ({03} Х s3c)

		tempCell[3] = ((xtime(stateMatrix[0][i]) ^ stateMatrix[0][i])	// ({03} Х s0c) (s1c) (s2c) ({02} Х s3c)
			^ stateMatrix[1][i]
			^ stateMatrix[2][i]
